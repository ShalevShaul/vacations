...
function getAllVacationsAsync() {
    return dal.executeQueryAsync('SELECT * FROM vacations');
}
...
function editVacationAsync(vacation_id, destination, description, start_time, end_time, price) {
    return dal.executeQueryAsync(
        ...
        price = ?
        WHERE vacation_id = ?`,
        [destination, description, start_time, end_time, price, vacation_id]
    );
}
...
function editVacationWithImageAsync(vacation_id, destination, description, start_time, end_time, price, image) {
    return dal.executeQueryAsync(
        ...
        image = ?
        WHERE vacation_id = ?`,
        [destination, description, start_time, end_time, price, image, vacation_id]
    );
}

function insertVacationAsync(destination, description, start_time, end_time, price, image) {
    return dal.executeQueryAsync(
        'INSERT INTO vacations (destination, description, start_time, end_time, price, image) VALUES (?, ?, ?, ?, ?, ?)',
        [destination, description, start_time, end_time, price, image]
    );
}

function getImageByVacationId(vacation_id) {
    return dal.executeQueryAsync(
        'SELECT image FROM vacations WHERE vacation_id = ?',
        [vacation_id]
    );
}
